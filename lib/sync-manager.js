...
class SyncManager {
  ...
  async start(config, apiKeyIndex = 0) {
    ...
    // Параметры конкретного ключа (или глобальные если не заданы)
    const keyYearStart = (typeof keyConfig === 'object' && keyConfig.yearStart) ? keyConfig.yearStart : (config.yearRangeStart || 2020);
    const keyYearEnd = (typeof keyConfig === 'object' && keyConfig.yearEnd) ? keyConfig.yearEnd : (config.yearRangeEnd || 2026);
